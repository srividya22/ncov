...
rule prefilter:
    ...
    input:
        ...
        metadata = config["metadata"],
    output:
        sequences = OUTDIR + "/results/prefiltered.fasta"
    log:
        OUTDIR + "/logs/prefiltered.txt"
    params:
...
rule refilter:
    message:
        ...
        """
    input:
        sequences = OUTDIR + "/results/aligned.fasta",
        metadata = config["metadata"],
        exclude = OUTDIR + "/results/to-exclude.txt"
    output:
...
rule mask:
    message:
        ...
        """
    input:
        alignment = OUTDIR + "/results/aligned-filtered.fasta"
    output:
...
rule filter:
    message:
        ...
        """
    input:
        sequences = OUTDIR + "/results/masked.fasta",
        metadata = config["metadata"],
...
rule subsample:
    ...
    log:
        OUTDIR + "/logs/subsample_{build_name}_{subsample}.txt"
    params:
...
rule proximity_score:
    message:
        ...
        """
    input:
        alignment = OUTDIR + "/results/filtered.fasta",
        metadata = config["metadata"],
        reference = config["files"]["reference"],
...
rule clade_files:
    ...
    output:
        OUTDIR + "/results/{build_name}/clades.tsv"
    shell:
        '''
...
rule subclades:
    ...
    output:
        clade_data = OUTDIR + "/results/{build_name}/temp_subclades.json"
    params:
        clade_file = OUTDIR + "/results/{build_name}/temp_subclades.tsv"
    log:
        OUTDIR + "/logs/subclades_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule rename_subclades:
    ...
    output:
        clade_data = OUTDIR + "/results/{build_name}/subclades.json"
    run:
        import json
...
rule nucleotide_mutation_frequencies:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        frequencies = OUTDIR + "/results/{build_name}/nucleotide_mutation_frequencies.json"
    log:
        OUTDIR + "/logs/nucleotide_mutation_frequencies_{build_name}.txt"
    params:
        min_date = config["frequencies"]["min_date"],
...
rule export:
    ...
    output:
        ...
        root_sequence_json = OUTDIR + "/results/{build_name}/ncov_with_accessions_root-sequence.json"
    log:
...
rule finalize:
    ...
    output:
        ...
        root_sequence_json = OUTDIR + "/auspice/ncov_{build_name}_root-sequence.json"
    log:
