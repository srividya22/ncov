...
def get_pangolin_lineages_from_file(cfile):
    d = pd.read_csv(cfile, header=0)
    lineages = {}
    for ri, row in d.iterrows():
        if row['lineage']:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--clade', type=str, required=False, help="clade file")
    parser.add_argument('--output', type=str, metavar="JSON", required=True, help="output Auspice JSON")
    args = parser.parse_args()

    if args.clade:
       lineages = get_pangolin_lineages_from_file(args.clade)
       all_lineages = get_pangolin_lineages()
