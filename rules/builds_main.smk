rule download:
    ...
    input:
        in_seq =  OUTDIR + "final/sequences.fasta",
        in_meta = OUTDIR + "final/metadata.tsv"
    output:
    ...
    shell:
        """
        cp {input.in_seq} {output.sequences:q}
        cp {input.in_meta} {output.metadata:q}
...
rule filter:
    ...
    output:
        sequences = OUTDIR + "results/filtered.fasta"
    log:
        OUTDIR + "logs/filtered.txt"
    params:
...
rule excluded_sequences:
    ...
    output:
        sequences = OUTDIR + "results/excluded.fasta"
    log:
        OUTDIR + "logs/excluded.txt"
    conda: config["conda_environment"]
    shell:
        """
        augur filter \
...
rule align_excluded:
    ...
    output:
        alignment = OUTDIR + "results/excluded_alignment.fasta"
    log:
        OUTDIR + "logs/align_excluded.txt"
    threads: 16
    conda: config["conda_environment"]
    shell:
        """
...
rule diagnose_excluded:
    ...
    input:
        ...
        reference = config["files"]["reference"]
    output:
        diagnostics = OUTDIR + "results/excluded-sequence-diagnostics.tsv",
        flagged = OUTDIR + "results/excluded-flagged-sequences.tsv",
        to_exclude = OUTDIR + "results/check_exclusion.txt"
    log:
        OUTDIR + "logs/diagnose-excluded.txt"
    params:
...
checkpoint partition_sequences:
    ...
    output:
        split_sequences = directory(OUTDIR + "results/split_sequences/")
    log:
        OUTDIR + "logs/partition_sequences.txt"
    params:
...
rule align:
    message:
        ...
        """
    input:
        sequences = OUTDIR + "results/split_sequences/{cluster}.fasta",
        reference = config["files"]["reference"]
    output:
        alignment = OUTDIR + "results/split_alignments/{cluster}.fasta"
    log:
        OUTDIR + "logs/align_{cluster}.txt"
    benchmark:
...
def _get_alignments(wildcards):
    ...
    return expand(OUTDIR + "results/split_alignments/{i}.fasta",
...
rule aggregate_alignments:
    ...
    output:
        alignment = OUTDIR + "results/aligned.fasta"
    log:
        OUTDIR + "logs/aggregate_alignments.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule diagnostic:
    ...
    input:
        ...
        reference = config["files"]["reference"]
    output:
        diagnostics = OUTDIR + "results/sequence-diagnostics.tsv",
        flagged = OUTDIR + "results/flagged-sequences.tsv",
        to_exclude = OUTDIR + "results/to-exclude.txt"
    log:
        OUTDIR + "logs/diagnostics.txt"
    params:
        mask_from_beginning = config["mask"]["mask_from_beginning"],
        mask_from_end = config["mask"]["mask_from_end"]
    conda: config["conda_environment"]
...
rule refilter:
    ...
    #    include_where = config["filter"]["include_where"]
    output:
        sequences = OUTDIR + "results/aligned-filtered.fasta"
    log:
        OUTDIR + "logs/refiltered.txt"
    conda: config["conda_environment"]
    shell:
        """
        augur filter \
...
def get_priorities(wildcards):
    ...
    if "priorities" in subsampling_settings and subsampling_settings["priorities"]["type"] == "proximity":
        return OUTDIR + f"results/{wildcards.build_name}/proximity_{subsampling_settings['priorities']['focus']}.tsv"
    else:
...
rule subsample:
    ...
    output:
        sequences = OUTDIR + "results/{build_name}/sample-{subsample}.fasta"
    params:
...
rule proximity_score:
    ...
    input:
        ...
        metadata = rules.download.output.metadata,
        reference = config["files"]["reference"],
        focal_alignment = OUTDIR + "results/{build_name}/sample-{focus}.fasta"
    output:
        priorities = OUTDIR + "results/{build_name}/proximity_{focus}.tsv"
    log:
        OUTDIR + "logs/subsampling_priorities_{build_name}_{focus}.txt"
    resources:
        mem_mb = 4000
    conda: config["conda_environment"]
    shell:
...
def _get_subsampled_files(wildcards):
    subsampling_settings = _get_subsampling_settings(wildcards)
    print(subsampling_settings)
    print(wildcards.build_name)
    print([ subsample for subsample in subsampling_settings])
    return [
        OUTDIR + f"results/{wildcards.build_name}/sample-{subsample}.fasta"
...
rule combine_samples:
    ...
    output:
        alignment = OUTDIR + "results/{build_name}/subsampled_alignment.fasta"
    log:
        OUTDIR + "logs/subsample_regions_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule adjust_metadata_regions:
    ...
    input:
        metadata = rules.download.output.metadata
    output:
        metadata = OUTDIR + "results/{build_name}/metadata_adjusted.tsv"
    params:
        region = lambda wildcards: config["builds"][wildcards.build_name]["region"]
    log:
        OUTDIR + "logs/adjust_metadata_regions_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule tree:
    ...
    output:
        tree = OUTDIR + "results/{build_name}/tree_raw.nwk"
    params:
        args = lambda w: config["tree"].get("tree-builder-args","") if "tree" in config else ""
    log:
        OUTDIR + "logs/tree_{build_name}.txt"
    benchmark:
...
rule refine:
    ...
    output:
        tree = OUTDIR + "results/{build_name}/tree.nwk",
        node_data = OUTDIR + "results/{build_name}/branch_lengths.json"
    log:
        OUTDIR + "logs/refine_{build_name}.txt"
    benchmark:
...
rule ancestral:
    ...
    input:
        ...
        alignment = rules.combine_samples.output.alignment
    output:
        node_data = OUTDIR + "results/{build_name}/nt_muts.json"
    log:
        OUTDIR + "logs/ancestral_{build_name}.txt"
    params:
...
rule haplotype_status:
    ...
    output:
        node_data = OUTDIR + "results/{build_name}/haplotype_status.json"
    log:
        OUTDIR + "logs/haplotype_status_{build_name}.txt"
    params:
...
rule translate:
    ...
    input:
        ...
        reference = config["files"]["reference"]
    output:
        node_data = OUTDIR + "results/{build_name}/aa_muts.json"
    log:
        OUTDIR + "logs/translate_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
        augur translate \
...
rule traits:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        node_data = OUTDIR + "results/{build_name}/traits.json"
    log:
        OUTDIR + "logs/traits_{build_name}.txt"
    params:
...
rule clades:
    ...
    output:
        clade_data = OUTDIR + "results/{build_name}/clades.json"
    log:
        OUTDIR + "logs/clades_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule pangolin:
    ...
    input:
        tree = rules.refine.output.tree,
        pangolin_clades = config["pangolin_clades"],
    output:
        clade_data = OUTDIR + "results/{build_name}/pangolin.json"
    log:
        OUTDIR + "logs/pangolin_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
        {python:q} scripts/add_pangolin_lineages_local.py \
            --tree {input.tree} \
            --clade {input.pangolin_clades} \
...
rule legacy_clades:
    ...
    output:
        clade_data = OUTDIR + "results/{build_name}/temp_legacy_clades.json"
    log:
        OUTDIR + "logs/legacy_clades_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
        augur clades --tree {input.tree} \
...
rule rename_legacy_clades:
    ...
    output:
        clade_data = OUTDIR + "results/{build_name}/legacy_clades.json"
    run:
        import json
...
rule colors:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        colors = OUTDIR + "results/{build_name}/colors.tsv"
    log:
        OUTDIR + "logs/colors_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule recency:
    ...
    input:
        metadata = _get_metadata_by_wildcards
    output:
        node_data = OUTDIR + "results/{build_name}/recency.json"
    log:
        OUTDIR + "logs/recency_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule tip_frequencies:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        tip_frequencies_json = OUTDIR + "results/{build_name}/tip-frequencies.json"
    log:
        OUTDIR + "logs/tip_frequencies_{build_name}.txt"
    params:
...
rule export:
    ...
    output:
        auspice_json = OUTDIR + "results/{build_name}/ncov_with_accessions.json"
    log:
        OUTDIR + "logs/export_{build_name}.txt"
    params:
        title = export_title
    conda: config["conda_environment"]
    shell:
...
rule incorporate_travel_history:
    ...
    output:
        auspice_json = OUTDIR + "results/{build_name}/ncov_with_accessions_and_travel_branches.json"
    log:
        OUTDIR + "logs/incorporate_travel_history_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
...
rule finalize:
    ...
    log:
        OUTDIR + "logs/fix_colorings_{build_name}.txt"
    conda: config["conda_environment"]
    shell:
        """
