...
rule subsample_focus:
    ...
    input:
        ...
        include = config["files"]["include"]
    output:
        sequences = OUTDIR + REGION_PATH + "subsample_focus.fasta"
    log:
...
rule make_priorities:
    ...
    output:
        priorities = OUTDIR + REGION_PATH + "subsampling_priorities.tsv"
    log:
...
rule subsample_context:
    ...
    output:
        sequences = OUTDIR + REGION_PATH + "subsample_context.fasta"
    log:
...
rule subsample_regions:
    ...
    output:
        alignment = OUTDIR + REGION_PATH + "subsampled_alignment.fasta"
    log:
...
rule adjust_metadata_regions:
    ...
    input:
        metadata = rules.download.output.metadata
    output:
        metadata = OUTDIR + REGION_PATH + "metadata_adjusted.tsv"
    log:
...
rule tree:
    ...
    output:
        tree = OUTDIR + REGION_PATH + "tree_raw.nwk"
    log:
...
rule refine:
    ...
    output:
        tree = OUTDIR + REGION_PATH + "tree.nwk",
        node_data = OUTDIR + REGION_PATH + "branch_lengths.json"
    log:
...
rule ancestral:
    ...
    input:
        ...
        alignment = _get_alignments_for_tree
    output:
        node_data = OUTDIR + REGION_PATH + "nt_muts.json"
    log:
...
rule haplotype_status:
    ...
    output:
        node_data = OUTDIR + REGION_PATH + "haplotype_status.json"
    log:
...
rule translate:
    ...
    input:
        ...
        reference = config["files"]["reference"]
    output:
        node_data = OUTDIR + REGION_PATH + "aa_muts.json"
    log:
...
rule traits:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        node_data = OUTDIR + REGION_PATH + "traits.json",
    log:
...
rule clades:
    ...
    output:
        clade_data = OUTDIR + REGION_PATH + "clades.json"
    log:
...
rule recency:
    ...
    input:
        metadata = _get_metadata_by_wildcards
    output:
        OUTDIR + REGION_PATH + "recency.json"
    log:
...
rule tip_frequencies:
    ...
    input:
        ...
        metadata = _get_metadata_by_wildcards
    output:
        tip_frequencies_json = OUTDIR + "auspice/ncov_{region}_tip-frequencies.json"
    log:
...
rule export:
    ...
    output:
        auspice_json = OUTDIR + REGION_PATH + "ncov_with_accessions.json"
    log:
...
rule incorporate_travel_history:
    ...
    output:
        auspice_json = OUTDIR + REGION_PATH + "ncov_with_accessions_and_travel_branches.json"
    log:
...
rule fix_colorings:
    ...
    output:
        auspice_json = OUTDIR + "auspice/ncov_{region}.json"
    log:
